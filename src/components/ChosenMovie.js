        ...
        <section className='chosen-movie' data-testid='chosen-movie'>
          <section className='backdrop-img'style={{ backgroundImage: `linear-gradient(to right, black, 55%, transparent), url(${movie.backdrop_path})`}}>
            <section className="movie-info">
              <Link to='/'>
              <img data-testid='return-btn' className="back-button-icon" src={backButton} alt={movie.title}/>
              </Link>
              <h3>{movie.title}</h3>
            ...
            <section className="video-frame">
              <iframe className="video-container" src={`https://youtube.com/embed/${video.key}`} title={movie.title} height="400" width="700"></iframe>
            </section>
          </section>
        </section>
      )
