        ...
        <section className='backdrop-img' style={{ backgroundImage: `linear-gradient(to right, black, 55%, transparent), url(${movie.backdrop_path})` }}>
          <section className="movie-info">
            ...
            <section className='monetary-info'>
              <p className='budget-text'>Budget: ${formatMonetaryInfo(movie.budget)}M</p>
              <p className='revenue-text'>Revenue: ${formatMonetaryInfo(movie.revenue)}M</p>
            </section>
            <p>Tagline: {movie.tagline}</p>
          </section>
          <section className="video-frame">
            {video && <iframe className="video-container" src={`https://youtube.com/embed/${video.key}`} alt='trailer-iframe-video-player' title='trailer-video-player' height="400" width="700"></iframe>}
          </section>
        </section>
      </section>
    )
